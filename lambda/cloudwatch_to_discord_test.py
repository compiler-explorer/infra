...
EXAMPLE_ANOMALY_EVENT = dict(
    ...
    NewStateReason="Thresholds Crossed: 7 out of the last 10 datapoints were "
                   ...
                   "OK -> ALARM transition).",
    StateChangeTime="2020-10-11T09:53:25.070+0000", Region="US East (N. Virginia)",
    ...
    Trigger=dict(
        ...
        EvaluateLowSampleCountPercentile="",
        Metrics=[
            dict(Id="m1", MetricStat=dict(
                Metric=dict(Dimensions=[
                    dict(value="app/GccExplorerApp/4a4513393260c7c5",
                         name="LoadBalancer")], MetricName="RequestCount",
                    Namespace="AWS/ApplicationELB"),
                Period=300, Stat="Sum"), ReturnData=True),
            dict(Expression="ANOMALY_DETECTION_BAND(m1,5)", Id="e1",
                 Label="RequestCount (Expected)", ReturnData=True)]))
