...
resource "aws_route53_zone" "compiler-explorer-com" {
  name = "compiler-explorer.com"
}

resource "aws_route53_record" "compiler-explorer-com" {
  for_each = {
    a    = "A"
    aaaa = "AAAA"
  }
  zone_id  = aws_route53_zone.compiler-explorer-com.zone_id
  name     = ""
  type     = each.value
  alias {
    name                   = aws_cloudfront_distribution.compiler-explorer-com.domain_name
    zone_id                = aws_cloudfront_distribution.compiler-explorer-com.hosted_zone_id
    evaluate_target_health = false
  }
}

resource "aws_route53_record" "star-compiler-explorer-com" {
  zone_id = aws_route53_zone.compiler-explorer-com.zone_id
  name    = "*"
  type    = "CNAME"
  ttl     = 60
  records = [aws_route53_zone.compiler-explorer-com.name]
}

resource "aws_route53_record" "spf-compiler-explorer-com" {
  zone_id = aws_route53_zone.compiler-explorer-com.zone_id
  name    = ""
  type    = "SPF"
  ttl     = 3600
...
resource "aws_route53_record" "mail-compiler-explorer-com" {
  name    = ""
  type    = "MX"
  ttl     = 3600
  zone_id = aws_route53_zone.compiler-explorer-com.zone_id
...
resource "aws_route53_record" "compiler-explorer-com-acm" {
  for_each = {
  for dvo in aws_acm_certificate.godbolt-org-et-al.domain_validation_options : dvo.domain_name => {
    name   = dvo.resource_record_name
    record = dvo.resource_record_value
