...
def shim_gcc_shell(install_dir: Path, gcc: Path, c_includes: str, cpp_includes: str, version: str):
    return f"""#!/bin/bash
set -euo pipefail
...
class EdgCompilerInstallable(NonFreeS3TarballInstallable):
    ...
    def stage(self, staging: StagingDir) -> None:
        ...
        def _call(command):
            ...
            subprocess.check_call(command, cwd=unzip_dir)
        ...
        def _query(lang: str, query_type: str) -> str:
            ...
            _LOGGER.info("Running %s", shlex.join(command))
            return subprocess.check_output(command, cwd=unzip_dir).decode("utf-8").strip()
