...
def calculate_routing_changes(
    current_compilers: Dict[str, Dict[str, Any]], 
    existing_table: Dict[str, Dict[str, Any]],
    environment: str
    ...
    for compiler_id, compiler_data in current_compilers.items():
        ...
        if compiler_id not in existing_table:
            ...
            if (existing_data.get("queueName") != routing_info["queueName"] or 
                existing_data.get("environment") != environment or
                existing_data.get("routingType") != routing_info["routingType"] or
                existing_data.get("targetUrl") != routing_info["targetUrl"]):
...
def batch_write_items(items_to_write: Dict[str, Dict[str, Any]]) -> None:
    ...
    try:
        ...
        for i in range(0, len(item_list), batch_size):
            ...
            for compiler_id, routing_data in batch:
            ...
            response = dynamodb_client.batch_write_item(
                RequestItems={COMPILER_ROUTING_TABLE: put_requests}
            )
        ...
        for i in range(0, len(item_list), batch_size):
            ...
            for composite_key in batch:
                delete_requests.append({
                    "DeleteRequest": {
                        "Key": {"compilerId": {"S": composite_key}}
                    }
                })
            ...
            response = dynamodb_client.batch_write_item(
                RequestItems={COMPILER_ROUTING_TABLE: delete_requests}
            )
...
def update_compiler_routing_table(environment: str, version: Optional[str] = None, dry_run: bool = False) -> Dict[str, int]:
...
def get_routing_table_stats() -> Dict[str, Any]:
    ...
    try:
        ...
        for compiler_id, data in routing_data.items():
...
def migrate_legacy_entries() -> Dict[str, int]:
    ...
    Scans the table for entries without environment prefixes and migrates them.
    Uses the environment field in the item to determine the correct composite key.
    
    Returns:
        Dictionary with migration statistics
    """
    try:
        LOGGER.info("Starting migration of legacy routing entries")
        ...
        # Find legacy entries (those without '#' in the key)
        for page in page_iterator:
            for item in page.get("Items", []):
                composite_key = item.get("compilerId", {}).get("S")
                if composite_key and "#" not in composite_key:
                    # This is a legacy entry
                    environment = item.get("environment", {}).get("S", "unknown")
                    legacy_entries.append({
                        "old_key": composite_key,
                        "environment": environment,
                        "item": item
                    })
        
        LOGGER.info(f"Found {len(legacy_entries)} legacy entries to migrate")
        ...
        for i in range(0, len(legacy_entries), batch_size):
            batch = legacy_entries[i:i + batch_size]
            
            # Prepare write requests for new composite keys
            write_requests = []
            delete_requests = []
            ...
            for entry in batch:
                ...
                if environment == "unknown":
                    LOGGER.warning(f"Skipping entry {old_key} - unknown environment")
                    skipped_count += 1
                    continue
                
                # Create new composite key
                new_key = create_composite_key(environment, old_key)
                
                # Prepare new item with composite key
                new_item = dict(item)
                new_item["compilerId"] = {"S": new_key}
                
                write_requests.append({
                    "PutRequest": {"Item": new_item}
                })
                
                # Prepare deletion of old entry
                delete_requests.append({
                    "DeleteRequest": {
                        "Key": {"compilerId": {"S": old_key}}
                    }
                })
            ...
            try:
                ...
                if write_requests:
                    dynamodb_client.batch_write_item(
                        RequestItems={COMPILER_ROUTING_TABLE: write_requests}
                    )
                    LOGGER.info(f"Migrated batch of {len(write_requests)} entries")
                ...
                if delete_requests:
                    ...
                    LOGGER.info(f"Deleted batch of {len(delete_requests)} legacy entries")
                
                migrated_count += len(write_requests)
                
            except Exception as e:
                LOGGER.error(f"Failed to migrate batch: {e}")
                errors += len(batch)
        
        LOGGER.info(f"Migration completed: {migrated_count} migrated, {skipped_count} skipped, {errors} errors")
        ...
        raise CompilerRoutingError(f"Failed to migrate legacy entries: {e}") from e
