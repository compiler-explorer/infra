    ...
    try:
        ...
        for page in page_iterator:
            for item in page.get("Items", []):
                ...
                if composite_key and "#" not in composite_key:
                    ...
                    legacy_entries.append({"old_key": composite_key, "environment": environment, "item": item})
        ...
        for i in range(0, len(legacy_entries), batch_size):
            ...
            for entry in batch:
                ...
                write_requests.append({"PutRequest": {"Item": new_item}})

                # Prepare deletion of old entry
                delete_requests.append({"DeleteRequest": {"Key": {"compilerId": {"S": old_key}}}})

            try:
                # Write new entries
                if write_requests:
                    dynamodb_client.batch_write_item(RequestItems={COMPILER_ROUTING_TABLE: write_requests})
                    LOGGER.info(f"Migrated batch of {len(write_requests)} entries")

                # Delete old entries
                if delete_requests:
                    dynamodb_client.batch_write_item(RequestItems={COMPILER_ROUTING_TABLE: delete_requests})
        ...
        return {"migrated": migrated_count, "skipped": skipped_count, "errors": errors}
