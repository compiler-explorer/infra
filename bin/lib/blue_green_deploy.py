...
class BlueGreenDeployment:
    ...
    def get_listener_rule_arn(self) -> Optional[str]:
        ...
        if self.env == "beta":
            ...
            listeners = elb_client.describe_listeners(
                LoadBalancerArn=self._get_load_balancer_arn()
            )
                    ...
                    for rule in rules['Rules']:
                        if rule.get('Conditions'):
                            for condition in rule['Conditions']:
                                if (condition.get('Field') == 'path-pattern' and 
                                    '/beta*' in condition.get('Values', [])):
                                    return rule['RuleArn']
    ...
    def wait_for_targets_healthy(self, target_group_arn: str, instance_ids: List[str], 
    ...
    def switch_target_group(self, new_color: str) -> None:
        ...
        ssm_client.put_parameter(
            Name=f"/compiler-explorer/{self.env}/active-color",
            Value=new_color,
            Overwrite=True
        )
        ssm_client.put_parameter(
            Name=f"/compiler-explorer/{self.env}/active-target-group-arn",
            Value=new_tg_arn,
            Overwrite=True
        )
    ...
    def deploy(self, target_capacity: Optional[int] = None) -> None:
        ...
        healthy_targets = self.wait_for_targets_healthy(inactive_tg_arn, instances)
    ...
    def rollback(self) -> None:
        ...
        response = as_client.describe_auto_scaling_groups(
            AutoScalingGroupNames=[previous_asg]
        )
            ...
            try:
                ...
                if response['AutoScalingGroups']:
                    ...
                    status['asgs'][color] = {
                        ...
                        'healthy_instances': len([i for i in asg['Instances'] 
