...
EXCLUDE_PATTERN = re.compile(r"(node_modules/\.cache|\.git/|\.DS_Store|__pycache__|\.pyc$|/tests/|test\.js$|\.test\.js$)")
...
def create_deterministic_zip(source_path, output_path):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for item in source_path.iterdir():
            ...
            if item.name in ["node_modules", ".git", "coverage", ".nyc_output", 
                           ".venv", ".pytest_cache", ".ruff_cache", "__pycache__",
                           ".mypy_cache", "dist", "build", ".dist"]:
                continue
            ...
            if item.is_file():
                ...
                shutil.copytree(item, temp_path / item.name, ignore=shutil.ignore_patterns(
                    "*.test.js", "*.spec.js", "test", "tests", "__tests__", "coverage",
                    "*.md", "__pycache__", "*.pyc", ".git", ".venv"
                ))
