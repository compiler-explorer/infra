...
describe('buildQueueUrl', () => {
    ...
    describe('with color-specific queue URLs configured', () => {
        beforeEach(() => {
            process.env.SQS_QUEUE_URL_BLUE =
                'https://sqs.us-east-1.amazonaws.com/123456789012/beta-compilation-queue-blue.fifo';
            process.env.SQS_QUEUE_URL_GREEN =
                'https://sqs.us-east-1.amazonaws.com/123456789012/beta-compilation-queue-green.fifo';
        });
        ...
        test('preserves existing blue color in queue name when active is blue', () => {
            ...
            expect(queueUrl).toBe(
                'https://sqs.us-east-1.amazonaws.com/123456789012/staging-compilation-queue-blue.fifo',
            );
        });
        ...
        test('preserves existing green color in queue name when active is green', () => {
            ...
            expect(queueUrl).toBe(
                'https://sqs.us-east-1.amazonaws.com/123456789012/staging-compilation-queue-green.fifo',
            );
        });
    ...
    describe('edge cases', () => {
        ...
        test('handles queue names that contain "blue" but not as color suffix', () => {
            ...
            expect(queueUrl).toBe(
                'https://sqs.us-east-1.amazonaws.com/123456789012/bluetooth-processor-queue-green.fifo',
            );
        });
        ...
        test('handles queue names that contain "green" but not as color suffix', () => {
            ...
            expect(queueUrl).toBe(
                'https://sqs.us-east-1.amazonaws.com/123456789012/greenfield-project-queue-blue.fifo',
            );
        });
